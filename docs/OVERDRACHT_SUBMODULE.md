...
Deze module bestond in drie lijnen: deze productrepo, de gekopieerde map
`daadit_ai_mistral/` in de fork `DAADit/daadit`, en — dat bleek pas bij de
omzetting — een eigen, verder doorontwikkelde map in de productielijn
`adriedaadit/daadit`, die op productie stond en staat. Sinds `19.0.10.0.0`
staat het werk van alle drie hier; de laatste stap is de map in de
productielijn vervangen door een submodule die op de release van deze repo
gepind staat.
...
- Die workflow draait sinds 19.0.9.0.0 **niet meer automatisch op een tag**;
  taggen is dus veilig en start geen kopieer-PR's meer. Let op: een tag die
  ouder is dan die wijziging gebruikt nog de oude definitie.
...
git tag -a v19.0.10.0.0 -m "Alle drie de lijnen samengevoegd (OAS 711)"
git push origin v19.0.10.0.0
```
...
curl -fsSLO https://raw.githubusercontent.com/DAADit/daadit_ai_mistral/v19.0.10.0.0/tools/adopt_as_submodule.sh
...
hier, tag opnieuw, en draai dan verder. Dat is bij ronde 2 ook gebeurd: de map
in `adriedaadit/daadit` bleek 2872 diff-regels eigen werk te hebben.
Alleen als het verschil aantoonbaar niets functioneels bevat, mag `--force`
erbij — bewaar `deploy-only.diff` dan als bewijs bij de PR.
...
git -C daadit_ai_mistral checkout v19.0.10.0.0   # detached HEAD = de pin
git add .gitmodules daadit_ai_mistral
git commit -m "daadit_ai_mistral als submodule, gepind op v19.0.10.0.0"
```
...
git -C daadit_ai_mistral describe --tags   # → v19.0.10.0.0
...
Verwacht: `state = installed`, `installed_version = 19.0.10.0.0` en gelijk aan
`latest_version`. Staat `installed_version` nog op de oude waarde terwijl
`latest_version` 19.0.10.0.0 is, dan is de module gevonden maar niet
geüpgraded — start de upgrade van die module dan expliciet.

Extra bewijs dat de migraties gedraaid zijn:
...
Dat is hier al één keer misgegaan als waarschuwing én als praktijk: bij ronde 1
stond productie op `19.0.6.28.0` tegen `19.0.6.27.0` in de fork, en bij ronde 2
bleek productie op `19.0.7.3.0` te staan — met werk dat in geen van de andere
twee lijnen zat. Meet daarom altijd eerst `installed_version` op productie. Is
die boven `19.0.10.0.0` uitgekomen, dan is er weer een lijn ontstaan: zoek uit
welk werk erachter zit, breng dat hierheen, tag daarboven en pin op die nieuwe
tag.
